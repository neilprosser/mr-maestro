...
(fact "that apply fails when Pedantic fails to apply a configuration"
      ...
      (provided
       (http/simple-get "http://pedantic/envs" {:socket-timeout 15000}) => {:status 200
                                                                                :body "{\"environments\":[\"poke\"]}"}
       (http/simple-get "http://pedantic/envs/poke/apps/app/apply" {:socket-timeout 180000}) => {:status 500}))
...
(fact "that responses are returned when Pedantic applies a config"
      ...
      (provided
       (http/simple-get "http://pedantic/envs" {:socket-timeout 15000}) => {:status 200
                                                                                :body "{\"environments\":[\"poke\"]}"}
       (http/simple-get "http://pedantic/envs/poke/apps/app/apply" {:socket-timeout 180000}) => {:status 200}))
...
(fact "that getting configuration does the right thing for the happy path"
      ...
      (provided
       (http/simple-get "http://pedantic/envs/environment/apps/application" {:socket-timeout 15000})
       => {:status 200
           :body ..body..}
